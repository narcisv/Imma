...
bool SortidaMotor =  false;
int SortidaVariable = 0;
bool SortidaProva = 0;
int ComptaS = 0;
//▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒  
...
/*
if ( TopSegon == 1 ) {  ComptaS++;   }
if (  ComptaS == 8 ) {  SortidaMotor = !SortidaMotor;  digitalWrite (MotorSortidaDireccio, SortidaMotor);  ComptaS++;   SortidaVariable = 0; }
if (  ComptaS == 15 )  {  SortidaMotor = !SortidaMotor;  digitalWrite (MotorSortidaDireccio, SortidaMotor);  ComptaS++;  SortidaVariable = 0; }
if ( Top == 1 ) {  SortidaVariable = SortidaVariable+4;  analogWrite (MotorSortidaPwm, SortidaVariable);  }
if (ComptaS > 15 ) ComptaS = 0;
*/
/*
 if ( TopSegon == 1 ) SortidaMotor =  !SortidaMotor;  
 if (Top == 1)  {SortidaVariable++;   SortidaProva = !SortidaProva; }
 
 digitalWrite ( LedA ,SortidaProva ); 
 if ( SortidaVariable > 255 )   { SortidaVariable = 0;      }
digitalWrite (MotorSortidaDireccio, SortidaMotor); analogWrite (MotorSortidaPwm, SortidaVariable);   // motor rotacio cinta
*/
...
if ( BotoH == false && ParoManual == false ) {
                                                           digitalWrite( MotorDcPlus1,HIGH  );  digitalWrite ( MotorDcPlus2,LOW   ); digitalWrite ( MotorDcPwm1,LOW   );
                                                            ...
                                                            if ( ComptManual2 > 4   ) {   analogWrite( MotorDcPwm2,128 ); ArribatAZero = true ;  }
                                                             if ( ComptManual2 > 6   )  {   digitalWrite (MotorSortidaDireccio, LOW ); analogWrite (MotorSortidaPwm,180); }  // motor rotacio cinta

                                                          if ( Top == true )  { ComptManual2++; constrain (ComptManual2 , 0, 50); }
                                                          if (ComptManual2 == 23) { digitalWrite ( MotorDcPlus1,HIGH  ) ; analogWrite ( MotorDcPwm2,75 ) ;    }
                                                           // if (ComptManual2 == 26) { digitalWrite ( MotorDcPlus1,HIGH  ) ; analogWrite ( MotorDcPwm2,35 ) ;    }                
                                                         // if (ComptManual2 == 27) { digitalWrite ( MotorDcPlus1,HIGH  ) ; analogWrite ( MotorDcPwm2,10 ) ;    }                                          
                                                          if (ComptManual2 == 25) { ParoManual = true ; ComptFrenada = 0 ; 
 ...
 //if ( TopSegon == 1 ) SortidaMotor =  !SortidaMotor;  
// if (Top == 1)  SortidaVariable++;  
// if ( SortidaVariable > 255 ) SortidaVariable = 0;                              
//digitalWrite (MotorSortidaDireccio, LOW ); analogWrite (MotorSortidaPwm,180);   // motor rotacio cinta
...
//ConsolaB();
 ...
 //▄▄▄▄▄▄▄▄▄▄▄▄▄▄ Baixada frenant  ▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
void FrenadaMotorDc(){
if ( BotoH == true  && BaixadaFreno == true){ //ConsolaB();
...
if (ComptFrenada <16)  {  digitalWrite( MotorDcPlus2,HIGH  ); analogWrite( MotorDcPwm1,60 ); } 
if (ComptFrenada == 17) {   
                                     digitalWrite ( MotorDcPlus1,LOW  );  digitalWrite ( MotorDcPlus2,LOW );
                                     digitalWrite( MotorDcPwm1,HIGH);  digitalWrite( MotorDcPwm2,HIGH );       }
 ...
 if (ComptFrenada >19)          {   
                                    digitalWrite ( MotorDcPlus1,LOW  );  digitalWrite ( MotorDcPlus2,LOW );
                                   analogWrite( MotorDcPwm1,255);  analogWrite( MotorDcPwm2,150 );       }                             
                                     
       if ( MicroElevacio == true  && ArribatAZero == true )    {
                                                        digitalWrite ( MotorDcPlus1,LOW  );  digitalWrite ( MotorDcPlus2,LOW );
                                                        digitalWrite( MotorDcPwm2,HIGH );  digitalWrite( MotorDcPwm1,HIGH );    
                                                        digitalWrite (MotorSortidaPwm, LOW);
                                                        digitalWrite( PowerMotorDc,LOW ); 
                                                        ArribatAZero = false;  BaixadaFreno = false; 
                                                        ComptFrenada = 0; ParoManual = false; 
                                                        ComptManual2 = 0;
                                                                                    }                                                  
                                                                           }
